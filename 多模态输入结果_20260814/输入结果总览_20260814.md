...
flowchart TD
    ...
    subgraph RECOGNITION["识别层：只识别内容与位置"]
        DETECT --> PARSERS["OpenDataLoader Hybrid / Docling / openpyxl / PyMuPDF"]
    ...
    subgraph INDEXING["索引层：按需把 visual 变成 Pixel 输入"]
        ...
        PDFCROP --> QUALITY
        RAWASSET --> QUALITY
        QUALITY --> PIXEL["常驻 PixelVisionProcessor<br/>只接收 visual 的像素资产"]
...
Word 后台 COM 原生导出可能返回空路径或因 Windows 登录会话不可用而直接失败。索引物化器现会捕获两种情况，复用识别阶段的版面 PDF，严格按已识别对象的 page+bbox 裁取后备区域。当前 `测试word.docx` 实测 11 个 visual 中有 9 个通过质量门形成有效 Pixel 输入（包括第 4 页折线图/飞机所在区域），不再产生“识别到 visual 但视觉索引为 0”的情况。
...
- OpenDataLoader 失败时自动回退 PyMuPDF。
...
|---|---|---|
| 格式检测 | 扩展名和文件签名 | 已完成 |
| PDF 结构解析 | OpenDataLoader PDF + PyMuPDF 回退 | 已验证 |
